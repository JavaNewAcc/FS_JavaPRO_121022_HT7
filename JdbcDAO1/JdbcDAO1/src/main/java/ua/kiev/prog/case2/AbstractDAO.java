...
public abstract class AbstractDAO<T> {
        ...
        try {
            ...
            for (Field f : fields) {
                if (f != id) {
                    ...
                    sb.append(f.getName()).append(" = ").append('"').append(f.get(t)).append('"').append(',');
                }
            }
    ...
    public List<T> getAll(Class<T> cls, String... parameter) {
        List<T> res = new ArrayList<>();
        StringBuilder requestedParameters = new StringBuilder();
        for (String param : parameter) {
            requestedParameters.append(param).append(",");
        }
        requestedParameters.deleteCharAt(requestedParameters.length() - 1);

        String sql = "SELECT " + requestedParameters + " FROM ";

        try {
            try (Statement st = conn.createStatement()) {
//                try (ResultSet rs = st.executeQuery("SELECT * FROM " + table)) {
                try (ResultSet rs = st.executeQuery(sql + table)) {
